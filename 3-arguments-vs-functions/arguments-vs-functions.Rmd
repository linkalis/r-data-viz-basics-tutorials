...
```{r, echo=FALSE}
# Data source: http://opendata.minneapolismn.gov/datasets/d050496ced314752bf89763139bc0cb3_0
data <- read.csv("Benchmarking_Results__CY_2012_Public_Buildings.csv", header=TRUE, na.strings = c("Not Available", "N/A"))
```
...
Another common scenario where assignment is used frequently is when you're creating new columns and tacking them onto your dataset.  For example, let's say you want to calculate how many years ago each building in the dataset was built by subtracting its Year\_Built from the current year.  You then want to save the result as a variable, 'years_ago', into your dataset:
...
This new variable gets appended to the data frame, so you won't actually see it as a separate object showing up in the "Environment" tab.  Instead, it's simply tacked on silently at the end of your table.  You should see, however, that the "Environment" tab displays that the number of varibles in your dataset has increased each time you create a new variable and assign it to a new column:
