...
```{r, echo=FALSE}
data <- read.csv("mpls_benchmarking_2014.csv", header=TRUE, na.strings=c("N/A", "Not Available", "NA"))
```
...
You can see above that the data appears somewhat multimodal--or just generally kind of distorted, with more observations near the low end and the high end of the histogram.  It is not a nice, smooth curve that peaks in the middle.  Again, you can use the `table()` function to understand a little more about what's going on.  Looking at the results of `table`, you can get a count of how many observations in the dataset fall under each particular score:
...
You can see, then, that a total of 143 buildings in the dataset are missing an Energy Star score.  That means that over half of the buildings on the dataset do not have an Energy Star score, and will not be included in the Energy Star score histogram you just plotted!  This is important to keep in mind as you're analyzing your data; the presence of this many missing values is something you should make sure to mention when writing up any kind of report or analysis about the buildings' Energy Star scores. 
...
Now, looking at the plot above, you can see that there _does not_ appear to be a strong relationship between the year the buildings were built and their respective Energy Star scores.  Newer buildings _do not_ seem to have consistently higher scores than older buildings.  In fact, there are many new buildings with low Energy Star scores, and plenty of older buildings with high Energy Star scores.  Scatterplots, then, are a nice way to test theories you may have about the relationship between different variables and help you decide which theories to reject, and which theories are worth examining in further analysis.
...
3. Is there a relationship between the amount of floor area in a building ("floor\_area") and its greenhouse gase emissions ("total\_GHG\_emissions")?
