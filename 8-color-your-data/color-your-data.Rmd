...
```{r}
ggplot(data, aes(x=year_built, y=site_EUI, col=organization)) +
  geom_point()
```
...
R has a built-in color palette of 657 unique colors you can use when creating your graphics.  Run the following command to print the names of all of R's built-in colors to the console.  You can use any of these colors when choosing custom colors for your color scales.  Just make sure to surround them in quotes (""), and you can add any of these colors to the `values=c()` argument as shown in the manual color scale above:
```{r, echo=FALSE}
colors()
```
...
```{r}
ggplot(data, aes(x=organization, y=site_EUI, fill=organization)) +
  geom_boxplot()
```
