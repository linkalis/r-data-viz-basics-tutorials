...
```{r}
data <- read.csv("../datasets/mpls_energy_benchmarking_2014.csv", header=TRUE, na.strings=c("NA", "Not Available", "N/A", "#REF!"))
```
...
ggplot(data, aes(x=year_built, y=site_EUI)) +
  ...
  ggtitle("Scatterplot of year built vs. site EUI for Mpls. buildings (2014)")
```
...
```{r, warning=FALSE, message=FALSE}
ggplot(data, aes(x=organization, y=site_EUI)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
