...
```{r}
data <- read.csv("mpls_benchmarking_2014.csv", header=TRUE, na.strings=c("NA"))
```
...
Some graphics only need a single "x" value in the aesthetic.  For example, here's the frame for a plot that looks at one variable--the site energy use intensity ("site\_EUI") of the various buildings in our dataset:
```{r}
ggplot(data, aes(x=site_EUI))
```

Some graphics need both an "x" and a "y" value in the aesthetic.  For example, here's the frame for a plot comparing the year a building was built (explanatory variable) to its Energy Star score (response variable):
```{r}
...
ggplot(data, aes(x=year_built, y=energy_star_score)) +
  xlab("Year built") + 
  ylab("Energy Star score") +
  ggtitle("Scatterplot of year built vs. Energy Star scores for Mpls. buildings (2014)")
```
...
ggplot(data, aes(x=year_built, y=energy_star_score)) +
  xlab("Year built") + 
  ylab("Energy Star score") +
  ggtitle("Year built vs. Energy Star scores for Mpls. buildings (2014)") +
...
Note: If you're having trouble reading the labels, you may also want to adjust the angle of the X axis label text.  Remember, you can add the following to rotate the X axis label text: `theme(axis.text.x = element_text(angle = 45, hjust = 1))`
